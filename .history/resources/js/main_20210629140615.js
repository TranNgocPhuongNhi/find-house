...
function showSlides(n) {
  var i;
  var featureItem = document.getElementsByClassName("feature__item");
  var dots = document.getElementsByClassName("dot");
  if (n > featureItem.length) {currentIndex = 1}    
  if (n < 1) {currentIndex = featureItem.length}
  for (i = 0; i < featureItem.length; i++) {
      featureItem[i].style.display = "none";  
  }
  for (i = 0; i < dots.length; i++) {
      dots[i].className = dots[i].className.replace(" active_dot", "");
  }
  featureItem[currentIndex-1].style.display = "block";  
  dots[currentIndex-1].className += " active_dot";
}
