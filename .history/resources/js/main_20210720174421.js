...
let minValue = document.getElementById('range-left');
let maxValue = document.getElementById('range-right');
let getMinPrice = document.querySelector('.price-min');
let getMaxPrice = document.querySelector('.price-max');
let sliderTrack = document.querySelector('.slider-track');
let sliderMaxValue = document.getElementById('range-left').max;

let minGap = 0;

minValue.addEventListener('input', slideOne)
maxValue.addEventListener('input', slideTwo)

function slideOne() {
    if(parseInt(maxValue.value + 2500) - parseInt(minValue.value + 500) <= minGap) {
        minValue.value = parseInt(maxValue.value) - minGap;
    }
    getMinPrice.innerHTML = `$${minValue.value}`
    fillColor()
}

function slideTwo() {
    if(parseInt(maxValue.value + 2500) - parseInt(minValue.value) <= minGap) {
        maxValue.value = parseInt(minValue.value) + minGap;
    }
    getMaxPrice.innerHTML = `$${maxValue.value}`
    fillColor()
}
